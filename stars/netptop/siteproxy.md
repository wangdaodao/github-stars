...
5.  **编辑配置文件**：
    -   使用文本编辑器打开 `siteproxy/wrangler.jsonc` 文件,修改如下字段并保存:
         ...
         "proxy\_url": "https://your-proxy-domain.com", // 替换为你的代理服务器域名, 必须为https
...
7.  **配置文件修改**：
    -   打开并修改 `config.json` 文件，内容如下：
        
        {
           ...
           "token\_prefix": "/user-SetYourPasswordHere/",  // 设置网站密码，用于防止非法访问，首尾的斜杠必须保留。为空表示不设置密码
