...
-   感谢**关注公众号** 的读者们，你们的留言、点赞、分享和推荐等积极互动让内容更有温度😎。
...
> 💡 关键词配置教程见 配置详解 - 关键词配置  
> 💡 也可以不做筛选，完整推送所有热点（将 frequency\_words.txt 留空）
...
**💡 提示：** 想要**实时更新**的网页版？fork 后，进入你的仓库 Settings → Pages，启用 GitHub Pages。效果预览。
...
2.  frequency\_words.txt 为空时，导致新闻发送也为空的逻辑问题

-   修复后，当你选择 frequency\_words.txt 为空时，将**推送所有新闻**，但受限于消息推送大小限制，请做如下调整
...
**功能新增**：增加增量推送(在 main.py 头部配置 FOCUS\_NEW\_ONLY)，该开关只关心新话题而非持续热度，只在有新内容时才发通知。
...
2.  更新文档使用，因为近期更新了很多功能，而且之前的使用文档我偷懒写的简单（见下面的 ⚙️ frequency\_words.txt 配置完整教程）

### 2025/06/16 - v1.0.0

1.  增加了一个项目新版本更新提示，默认打开，如要关掉，可以在 main.py 中把 "FEISHU\_SHOW\_VERSION\_UPDATE": True 中的 True 改成 False 即可
...
**100 star⭐** 了，写个小功能给大伙儿助助兴 frequency\_words.txt 文件增加了一个【必须词】功能，使用 + 号
...
2.  **设置 GitHub Secrets（选择你需要的平台）**:
    ...
    **GitHub Secret 配置（⚠️ Name 名称必须严格一致）：**
    ...
    其中方案一，由 **ziventian**发现并提供建议，在这里感谢他，默认是个人推送，也可以配置群组推送操作#97 ，
    ...
    **GitHub Secret 配置（⚠️ Name 名称必须严格一致）：**
    ...
    -   **Secret（值）**：收件人邮箱地址（多个收件人用英文逗号分隔，也可以和 EMAIL\_FROM 一样，自己发送给自己）
    ...
    **说明**：邮件推送需要配置至少**3个必需** Secret（EMAIL\_FROM、EMAIL\_PASSWORD、EMAIL\_TO），后两个为可选配置
    ...
    **天翼邮箱**
    ...
    > **反馈说明**：
    > 
    > -   如果你使用**其他邮箱**测试成功，欢迎开 Issues 告知，我会添加到支持列表
    ...
    **如果有多个收件人(注意是英文逗号分隔)**：
    ...
    -   EMAIL\_TO="user1@example.com,user2@example.com,user3@example.com"
    ...
    **快速开始：**
    ...
    3.  **配置 GitHub Secret（⚠️ Name 名称必须严格一致）**：
        ...
        **说明**：ntfy 至少需要配置 1 个必需 Secret (NTFY\_TOPIC)，后两个为可选配置
    ...
    **配置 TrendRadar**：
    ...
    NTFY\_SERVER\_URL: https://ntfy.yourdomain.com
    NTFY\_TOPIC: trendradar-alerts  # 自托管可用简单名称
    NTFY\_TOKEN: tk\_your\_token  # 可选：启用访问控制
...
4.  **配置说明（可选）**：
    ...
    -   **关键词设置**：在 config/frequency\_words.txt 中添加你关心的关键词
...
👉 点击展开：**frequency\_words.txt 配置教程**  
...
weight:
  rank\_weight: 0.8    # 主要看排名
  frequency\_weight: 0.1  # 不太在乎持续性
  hotness\_weight: 0.1
...
weight:
  rank\_weight: 0.4    # 适度看排名
  frequency\_weight: 0.5  # 重视当天内的持续热度
  hotness\_weight: 0.1
...
1.  **三个数字加起来必须等于 1.0**
2.  **哪个重要就调大哪个**：在乎排名就调大 rank\_weight，在乎持续性就调大 frequency\_weight
...
2.  下载配置文件：
    ...
    -   访问 https://raw.githubusercontent.com/sansan0/TrendRadar/master/config/frequency\_words.txt → 右键"另存为" → 保存到 `config\frequency_words.txt`
...
docker run -d --name trend-radar \\
  ...
  -v ./output:/app/output \\
  -e FEISHU\_WEBHOOK\_URL="你的飞书webhook" \\
  -e DINGTALK\_WEBHOOK\_URL="你的钉钉webhook" \\
  -e WEWORK\_WEBHOOK\_URL="你的企业微信webhook" \\
  -e TELEGRAM\_BOT\_TOKEN="你的telegram\_bot\_token" \\
  -e TELEGRAM\_CHAT\_ID="你的telegram\_chat\_id" \\
  -e EMAIL\_FROM="你的发件邮箱" \\
  -e EMAIL\_PASSWORD="你的邮箱密码或授权码" \\
  -e EMAIL\_TO="收件人邮箱" \\
  -e CRON\_SCHEDULE="\*/30 \* \* \* \*" \\
  -e RUN\_MODE="cron" \\
  -e IMMEDIATE\_RUN="true" \\
...
vim config/frequency\_words.txt
...
3.  **在浏览器中连接**：
    ...
    -   检查 "List Tools" 是否返回 13 个工具：
        -   基础查询：get\_latest\_news, get\_news\_by\_date, get\_trending\_topics
        -   智能检索：search\_news, search\_related\_news\_history
        -   高级分析：analyze\_topic\_trend, analyze\_data\_insights, analyze\_sentiment, find\_similar\_news, generate\_summary\_report
        -   系统管理：get\_current\_config, get\_system\_status, trigger\_crawl
...
3.  查看详细错误日志：
    ...
    uv run python -m mcp\_server.server --transport http --port 3333
    
4.  尝试自定义端口:
    
    uv run python -m mcp\_server.server --transport http --port 33333
...
> 本项目 AI 分析功能需配置大模型使用，配置教程详见 AI 智能分析
...
flowchart TD
    ...
    F --> G\[📝 配置关键词<br/>config/frequency\_words.txt<br/>普通词/必须词+/过滤词!\]
